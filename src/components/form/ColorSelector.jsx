...
const ColorThemeDialog = ({ open, onClose, elementsInfo = {color: ''}, setElementsInfo }) => {
  ...
  const handleSave = () => {
    if (selectedColor) {
      ...
      console.error('Select a color before saving.');
    }
  };
      ...
      <DialogContent>
        <Grid container spacing={2}>
          {themeColors.map((color) => (
            <Grid item key={color} xs={3} display={"flex"} alignItems={"center"} justifyContent={"center"}>
              <IconButton
                style={{
